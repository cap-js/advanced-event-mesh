...
global.fetch = jest.fn((url, opts) => {
  if (!opts.method && url.match(/\/subscriptions$/)) {
    ...
    return Promise.resolve({
      json: () =>
        Promise.resolve({ data: [{ subscriptionTopic: "toBeDeleted" }] }),
    });
  }
  return Promise.resolve({
    status: 200,
...
jest.mock("https", () => {
  ...
  return {
    ...
    request: (url, opts, cb) => {
      ...
      res.push(
        JSON.stringify({ access_token: "<sampleToken>", expires_in: 1 }),
      );
      res.push(null);
...
describe("simple unit tests", () => {
  ...
  test("emit from app service", async () => {
    ...
    expect(check.sentMessages[0].binary).toBe(
      JSON.stringify({ data: DATA, ...HEADERS }),
    );
    expect(check.sentMessages[0].dest).toBe("foo");
    expect(check.sentMessages[0].mode).toBe("PERSISTENT");
    expect(check.sentMessages[1].binary).toBe(
      JSON.stringify({ data: DATA2, ...HEADERS2 }),
    );
  ...
  test('malformed payload "null" is settled, not leaked', (done) => {
    messaging.messageConsumer.emit("MESSAGE", {
      ...
      async acknowledge() {
          ...
          new Error(
            "Should not have acknowledged: malformed payload must settle",
          ),
        );
      },
      settle(e) {
  ...
  test("fresh new token", (done) => {
    setTimeout(() => {
      expect(
        messaging.session.updateAuthenticationOnReconnect,
      ).toHaveBeenCalled();
      done();
    }, 1000);
  });
